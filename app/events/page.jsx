    ...
    <div className="min-h-screen bg-gradient-to-b from-black to-blue-300 text-white">
      ...
      <main className="container mx-auto px-4 py-8 space-y-12">
        {events.map((event, index) => (
          ...
          >
            <h2 className="text-2xl md:text-3xl font-bold mb-6 text-blue-400">{event.E_name}</h2>
            <div className="md:flex md:space-x-6 space-y-6 md:space-y-0">
              ...
              <div className="md:w-1/2 space-y-4">
                <p><strong className="text-blue-300">Date:</strong> {new Date(event.E_date).toLocaleDateString()}</p>
                <p><strong className="text-blue-300">Time:</strong> {event.E_timings}</p>
                <p><strong className="text-blue-300">Location:</strong> {event.E_location}</p>
                <p><strong className="text-blue-300">Domain:</strong> {event.E_domain}</p>
                <p><strong className="text-blue-300">Team Size:</strong> {event.E_team_size}</p>
                <p className="text-gray-300">{event.E_mini_description}</p>
                <div className="flex flex-col sm:flex-row gap-4 mt-6">
                  <Link href={`/events/create-team/${event._id}`} passHref>
                    <motion.a
                      className="bg-blue-600 text-white font-semibold py-3 px-6 rounded-lg hover:bg-blue-500 transition-colors text-center shadow-md"
                  ...
                  <Link href={`/events/join-team/${event._id}`} passHref>
                    <motion.a
                      className="bg-green-600 text-white font-semibold py-3 px-6 rounded-lg hover:bg-green-500 transition-colors text-center shadow-md"
                      whileHover={{ scale: 1.05 }}
                      whileTap={{ scale: 0.95 }}
                    >
                      Join Team
      ...
      <footer className="w-full min-h-screen flex flex-col items-center justify-center text-center bg-gradient-to-b from-blue-900 to-black p-8">
        <h2 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-12">
          Ready to <span className="bg-gradient-to-r from-blue-400 via-green-300 to-indigo-400 text-transparent bg-clip-text">take flight</span>?
        </h2>
        <div className="relative w-64 h-64 md:w-80 md:h-80 lg:w-96 lg:h-96 flex items-center justify-center">
          ...
          >
            ...
            <text fill="#fff">
              <textPath xlinkHref="#circlePath" className="text-sm md:text-base lg:text-lg">
