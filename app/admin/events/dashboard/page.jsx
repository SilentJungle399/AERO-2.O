...
import { FaMapMarkerAlt, FaClock, FaCalendarAlt, FaMouse } from 'react-icons/fa';
      ...
      <div className="container mx-auto px-4 py-16">
        ...
        <div className="flex flex-col ">
          {sortedEvents.map(event => (
            <div key={event._id} className="bg-gray-800 rounded-lg overflow-hidden shadow-lg transition-all duration-300 hover:shadow-2xl hover:scale-105">
              <img src={event.E_main_img || "/default-event.jpg"} alt={event.E_name} className="w-full h-48 object-cover" />
              <div className="p-6">
                ...
                <div className="flex flex-col space-y-2 text-sm text-gray-300 mb-4">
                  ...
                  <p><FaCalendarAlt className="inline mr-2 text-green-500" />{new Date(event.E_date).toLocaleDateString()}</p>
        ...
        <div className='flex m-4  items-center justify-center'>
            <Link className='bg-green-500 font-bold p-4 rounded-lg font-mono transform transition-transform duration-300 ease-in-out hover:scale-110 ' href="/admin/events/create">Create Event</Link>
        </div>
      </div>
    </div>
  );
