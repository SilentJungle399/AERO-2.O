...
import {ArrowDown, Calendar, Clock, MapPin, Save, Users, X} from "lucide-react";
          ...
          <div className="bg-gray-800 w-11/12 md:max-w-3xl mx-auto rounded-lg shadow-xl overflow-hidden">
            ...
            <div className="p-6">
              <div className="grid md:grid-cols-2 gap-4 mb-6">
                <div>
                  <p className="text-gray-300"><strong className="text-gray-100">Group
                    Token:</strong> {team.Group_token}</p>
                  <p className="text-gray-300"><strong className="text-gray-100">Leader
                    Name:</strong> {team.Group_leader_id.full_name}</p>
                </div>
                <div>
                  <p className="text-gray-300"><strong className="text-gray-100">Leader
                    Email:</strong> {team.g_leader_email}</p>
                  <p className="text-gray-300"><strong className="text-gray-100">Leader
                    Mobile:</strong> {team.g_leader_mobile}</p>
                </div>
              </div>
              <div className="flex items-center mb-4">
                <Users className="w-5 h-5 mr-2 text-blue-400"/>
                <span
                  className="text-lg font-semibold text-gray-100">Team Members ({team.Group_members_team_ids.length})</span>
              </div>
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-700">
                  <thead className="bg-gray-700">
                  <tr>
                    <th
                      className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Name
                    </th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Roll
                      No
                    </th>
                    <th
                      className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Mobile
                      No
                    </th>
                    <th
                      className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Payment
                      Receipt
                    </th>
                    <th
                      className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Time
                    </th>
                  </tr>
                  </thead>
                  <tbody className="bg-gray-800 divide-y divide-gray-700">
                  {team.Group_members_team_ids.map((member) => (
                    <tr key={member._id}>
                      <td
                        className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-100">{member.Member_name}</td>
